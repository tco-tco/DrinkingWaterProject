...
def createDB(ilAdi):
    connection=mysql.connector.connect(host="localhost", user= "root",password="rnu9hodWT*****")
    cursor=connection.cursor()
    cursor.execute(f"CREATE DATABASE {ilAdi}")
def createTable(tesisAdi,ilAdi):
    connection=mysql.connector.connect(host="localhost", user= "root",password="rnu9hodWT*****", database=f"{ilAdi}")
    cursor=connection.cursor()
